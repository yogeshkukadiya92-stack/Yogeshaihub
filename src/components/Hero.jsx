      ...
      <div className="relative glass-card rounded-2xl p-6 border border-white/[0.07]">
          ...
          {workflowSteps.map((step, i) => (
            <div key={step.id}>
              ...
              >
                ...
                {active > i && (
                  <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-5 h-5 rounded-full bg-emerald-500/30 border border-emerald-500/50 flex items-center justify-center ml-auto">
              ...
              {i < 3 && (
                <div className="ml-[22px] my-1 flex flex-col gap-0.5">
                  {[0, 1, 2].map((d) => (
                    <motion.div key={d} className="w-0.5 h-1 rounded-full" animate={active > i ? { backgroundColor: '#6366f1' } : { backgroundColor: '#1e293b' }} transition={{ delay: d * 0.1 }} />
                  ))}
                </div>
              )}
            </div>
        ...
        <div className="mt-5 pt-4 border-t border-white/[0.05]">
          ...
          <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
            <motion.div className="h-full bg-gradient-to-r from-indigo-500 via-violet-500 to-cyan-500 rounded-full" animate={{ width: [`${(active / 3) * 100}%`] }} transition={{ duration: 0.4 }} />
          </div>
        </div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div variants={container} initial="hidden" animate="show">
            ...
            <motion.h1 variants={item} className="text-5xl sm:text-6xl lg:text-7xl font-black text-white leading-[1.05] tracking-tight">
              {h.headline.split(' ').slice(0, 3).join(' ')}{' '}
              <span className="gradient-text">{h.headline.split(' ').slice(3, 5).join(' ')},</span>{' '}
              {h.headline.split(' ').slice(5, 7).join(' ')}{' '}
              <span className="text-slate-400">&</span>{' '}
              <span className="gradient-text">{h.headline.split(' ').slice(8, 9).join(' ')}</span>{' '}
              {h.headline.split(' ').slice(9).join(' ')}
            </motion.h1>

            <motion.p variants={item} className="mt-6 text-lg text-slate-400 leading-relaxed max-w-lg">
              {h.description}
            </motion.p>

            <motion.div variants={item} className="mt-8 flex flex-wrap gap-3">
              <a href="#contact" className="group inline-flex items-center gap-2 px-6 py-3.5 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-semibold shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 hover:from-indigo-500 hover:to-violet-500 transition-all duration-300">
                {h.ctaPrimary}
                <ChevronRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" />
              </a>
              <a href="#services" className="inline-flex items-center gap-2 px-6 py-3.5 rounded-xl border border-white/10 text-slate-300 hover:text-white hover:border-white/20 hover:bg-white/5 transition-all duration-300 font-medium">
                {h.ctaSecondary} <ArrowRight className="w-4 h-4" />
              </a>
            </motion.div>

            <motion.div variants={item} className="mt-10 flex items-center gap-6">
              <a href={whatsappHref} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-sm text-slate-400 hover:text-emerald-400 transition-colors">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-sm text-slate-400">{h.trustText}</span>
              </div>
            </motion.div>
          </motion.div>
