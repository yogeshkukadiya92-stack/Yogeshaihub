  ...
  return (
    <section id="testimonials" className="py-24 border-t border-white/[0.04] relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.1 }} className="max-w-3xl mx-auto mb-12">
          <motion.div key={current} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }} className="rounded-2xl border border-white/[0.08] p-8 sm:p-10 relative overflow-hidden" style={{ background: '#0d1524' }}>
            ...
            <div className="relative z-10">
              ...
              <blockquote className="text-slate-300 text-lg leading-relaxed mb-6">"{testimonials[current].quote}"</blockquote>
              <div className="px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-sm font-semibold inline-block">
        ...
        <div className="flex items-center justify-center gap-4">
          <button onClick={prev} className="w-10 h-10 rounded-xl border border-white/[0.08] flex items-center justify-center text-slate-400 hover:text-white hover:border-white/20 transition-all">
            <ChevronLeft className="w-4 h-4" />
          </button>
          <div className="flex gap-2">
            {testimonials.map((_, i) => (
              <button key={i} onClick={() => setCurrent(i)} className={`rounded-full transition-all duration-300 ${i === current ? 'w-6 h-2 bg-indigo-500' : 'w-2 h-2 bg-white/20 hover:bg-white/40'}`} />
            ))}
          </div>
          <button onClick={next} className="w-10 h-10 rounded-xl border border-white/[0.08] flex items-center justify-center text-slate-400 hover:text-white hover:border-white/20 transition-all">
            <ChevronRight className="w-4 h-4" />
          </button>
        </div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.3 }} className="mt-14 grid md:grid-cols-3 gap-4">
          {testimonials.slice(0, 3).map((t, i) => (
            <button key={i} onClick={() => setCurrent(i)} className={`text-left rounded-xl p-4 border transition-all duration-300 ${current === i ? 'border-indigo-500/30 bg-indigo-900/10' : 'border-white/[0.05] hover:border-white/10'}`} style={{ background: current === i ? undefined : '#0d1524' }}>
