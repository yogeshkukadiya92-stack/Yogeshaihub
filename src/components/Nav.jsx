...
export default function Nav() {
  ...
  const links = siteData.navigation || [];
  const brand = siteData.brand || {};
  const whatsappHref = `https://wa.me/${siteData.contact.whatsapp}?text=${encodeURIComponent('Hi, I want to book a free AI strategy consultation.')}`;
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <a href="#hero" className="flex items-center gap-2.5 group">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center shadow-lg shadow-indigo-500/30 group-hover:shadow-indigo-500/50 transition-shadow overflow-hidden">
              {brand.logoUrl ? <img src={brand.logoUrl} alt="" className="w-full h-full object-cover" /> : <Brain className="w-4 h-4 text-white" />}
            </div>
            <span className="font-bold text-white text-lg tracking-tight">
              {brand.name || 'Yogesh AI Hub'}
            </span>
          </a>
