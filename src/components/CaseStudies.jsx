...
const card = { hidden: { opacity: 0, y: 30 }, show: { opacity: 1, y: 0, transition: { duration: 0.6, ease: [0.16, 1, 0.3, 1] } } };

export default function CaseStudies() {
  const ref = useRef(null);
  const inView = useInView(ref, { once: true, margin: '-60px' });
  const { siteData } = useSiteData();
  const cases = siteData.caseStudies;
  ...
  return (
    <section id="case-studies" className="py-24 border-t border-white/[0.04] relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div ref={ref} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="mb-14">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <h2 className="text-4xl sm:text-5xl font-black text-white leading-tight">
              Real Results,<br /><span className="gradient-text">Real Businesses</span>
            </h2>
        ...
        <motion.div variants={container} initial="hidden" animate={inView ? 'show' : 'hidden'} className="grid lg:grid-cols-3 gap-6">
          {cases.map((c, idx) => (
            <motion.div key={idx} variants={card} whileHover={{ y: -6, transition: { duration: 0.2 } }} className={`rounded-2xl border ${c.border} overflow-hidden group cursor-default flex flex-col`} style={{ background: '#0d1524' }}>
              <div className={`p-6 flex-1 bg-gradient-to-br ${c.gradient}`}>
                ...
                <div className="mb-4">
                  <div className="flex items-center gap-2 text-xs text-rose-300 font-semibold uppercase tracking-wide mb-2">
                    <div className="w-1 h-1 rounded-full bg-rose-400" /> Problem
                  </div>
                ...
                <div className="mb-4">
                  <div className="flex items-center gap-2 text-xs text-indigo-300 font-semibold uppercase tracking-wide mb-2">
                    <div className="w-1 h-1 rounded-full bg-indigo-400" /> Solution
                  </div>
                ...
                <div>
                  <div className="flex items-center gap-2 text-xs text-emerald-300 font-semibold uppercase tracking-wide mb-2">
                    <div className="w-1 h-1 rounded-full bg-emerald-400" /> Result
                  </div>
              ...
              <div className="border-t border-white/[0.05] p-5 bg-white/[0.01]">
                ...
                <button className="w-full py-2.5 rounded-lg bg-white/[0.03] border border-white/[0.06] text-slate-400 text-xs font-medium flex items-center justify-center gap-1.5 hover:text-white hover:border-white/10 transition-colors group/btn">
                  Read Full Case Study <ArrowRight className="w-3.5 h-3.5 group-hover/btn:translate-x-0.5 transition-transform" />
                </button>
              </div>
            </motion.div>
          ))}
