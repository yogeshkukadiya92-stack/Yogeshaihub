...
const card = { hidden: { opacity: 0, y: 24 }, show: { opacity: 1, y: 0, transition: { duration: 0.5, ease: [0.16, 1, 0.3, 1] } } };
...
function ServiceCard({ s }) {
  return (
    <motion.div
      ...
      className={`relative rounded-2xl p-6 border ${s.accent} overflow-hidden group cursor-default flex flex-col gap-4`}
    ...
    >
      ...
      <div className="relative z-10 mt-auto">
        <a href="#contact" className="text-xs text-slate-500 group-hover:text-indigo-400 transition-colors duration-300 flex items-center gap-1 font-medium">
  ...
  return (
    <section id="services" className="py-24 relative">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} ref={ref} className="mb-14">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <h2 className="text-4xl sm:text-5xl font-black text-white leading-tight">
              Every AI Solution<br /><span className="gradient-text">Your Business Needs</span>
            </h2>
        ...
        <motion.div variants={container} initial="hidden" animate={inView ? 'show' : 'hidden'} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 auto-rows-[180px]">
          {/* Large featured card — AI Consultancy */}
          <motion.div
            variants={card}
            whileHover={{ y: -4 }}
